...
/*   Updated: 2025/09/19 04:43:41 by eratasoy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_mandelbrot(int x, int y, t_fractol *fractol, char *line)
{
	...
	while (i < fractol->max_iteration)
	{
		...
		{
			zn = sqrt(z.re * z.re + z.img * z.img);
			nu = log(log(zn) / log(2)) / log(2);
			smooth_iter = i + 1 - nu;
			fractol->color = get_gradient_color(smooth_iter,
					fractol->max_iteration, fractol->type_of_fractal);
